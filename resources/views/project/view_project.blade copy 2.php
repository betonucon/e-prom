					...
					<input type="hidden" id="myurl" value="approve_kadis_komersil">
						...
						<div id="alasan">
							...
							<div class="input-group">
								...
								<textarea class="form-control form-control-sm"  name="catatan" value="" placeholder="Enter....." rows="4">{{$data->catatan}}</textarea>
							</div>
						</div>
				</form>
			</div>
        ...
        <script type="text/javascript">
			...
			$('#save-approve').on('click', () => {
				...
				@if(Auth::user()->role_id==2)
					var uril="{{ url('project/approve_direktur_operasional') }}";
				@endif
				@if(Auth::user()->role_id==3)
					var uril="{{ url('project/approve_mgr_operasional') }}";
				@endif
				@if(Auth::user()->role_id==4)
					var uril="{{ url('project/approve_kadis_komersil') }}";
				@endif
				@if(Auth::user()->role_id==7)
					var uril="{{ url('project/approve_kadis_operasional') }}";
				@endif
